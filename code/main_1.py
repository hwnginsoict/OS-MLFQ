...
auto_run_flag = False
simulation_speed = 1.0  # Thời gian delay giữa các bước (đơn vị: giây)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Create auto run checkbox
        # Label + Entry thời gian mô phỏng
        ctk.CTkLabel(self.sidebar_frame, text="Simulation Speed (s)").grid(row=3, column=0, padx=20, pady=(10, 0))
        self.speed_entry = ctk.CTkEntry(self.sidebar_frame, width=100)
        self.speed_entry.insert(0, str(simulation_speed))  # mặc định 1.0 giây
        self.speed_entry.grid(row=4, column=0, padx=20, pady=(0, 5))

        # Nút set tốc độ
        self.set_speed_btn = ctk.CTkButton(self.sidebar_frame, text="Set Speed", command=self._set_speed)
        self.set_speed_btn.grid(row=5, column=0, padx=20, pady=(0, 10))
        # Nút tự động chạy 
        self.autorun_btn = ctk.CTkButton(self.sidebar_frame, text="Auto Run", command=self._start_auto_run)
        self.autorun_btn.grid(row=6, column=0, padx=20, pady=(0, 10))
        # Nút dừng tự động chạy
        self.stop_btn = ctk.CTkButton(self.sidebar_frame, text="Stop Auto Run", command=self._stop_auto_run, state="disabled")
        self.stop_btn.grid(row=7, column=0, padx=20, pady=(0, 10))
        ...
        def submit():
            ...
            # Next button
            self.next_btn = ctk.CTkButton(self.main_frame, text="Next", command=next_timestep)
            self.next_btn.grid(row=0, column=3, padx=20, pady=20, sticky="ne")
    ...
    def _set_speed(self):
        ...
        try:
            ...
            if val < 0.05:
                val = 0.05  # tránh sleep quá ngắn
            simulation_speed = val
        except ValueError:
            pass  # nếu nhập sai thì bỏ qua
    ...
    def _run_loop(self):
        global auto_run_flag
        while auto_run_flag and not finish_check:
            self.next_btn.invoke()
            time.sleep(simulation_speed)

        # Khi kết thúc mô phỏng (finish_check == True)
        auto_run_flag = False
        self.autorun_btn.configure(state="normal")
        self.stop_btn.configure(state="disabled")
